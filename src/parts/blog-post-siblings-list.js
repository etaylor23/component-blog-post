...
export function siblingList(
  ...
  return (
    <div className={`blog-post__${ elementClassName }-aside`}>
      ...
      <span className={`blog-post-side-text`}>More in this {printSectionName.toLowerCase()}:</span>
      <ul className={`blog-post__${ elementClassName }-list`}>
        {articlesList.map((article, index) => {
          ...
          const firstLinkClassName = isCurrentArticleSelected ? `blog-post__${ elementClassName }-first-link` : '';
          return (
